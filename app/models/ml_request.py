...
class MLRequest(SQLModel, table=True):
    ...
    user_id: uuid.UUID = Field(sa_column=Column(SA_UUID, ForeignKey("user.id", name="fk_user_id")))
    ml_model_id: uuid.UUID = Field(sa_column=Column(SA_UUID, ForeignKey("ml_model.id", name="fk_ml_model_id")))
    data_id: uuid.UUID = Field(sa_column=Column(SA_UUID, ForeignKey("data.id", name="fk_ml_data_id")))
