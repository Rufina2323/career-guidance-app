...
def test_deposit(balance_psql_repository: BalancePSQLRepository) -> None:
    balance_id = uuid4()
    amount = 100
    ...
    with patch.object(balance_psql_repository, "session_maker") as mock_session_maker:
        ...
        mock_psql_balance.amount = 1000
        mock_session.exec.return_value.one.return_value = mock_psql_balance

        balance_psql_repository.deposit(balance_id, amount)
        ...
        assert mock_psql_balance.amount == 1100
