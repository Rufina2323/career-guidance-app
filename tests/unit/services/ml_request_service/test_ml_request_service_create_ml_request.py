...
def test_create_ml_request(ml_request_service: MLRequestService) -> None:
    ...
    with patch.object(ml_request_service.ml_model_service, "get_ml_model_request_cost", return_value=model_cost) as mock_get_cost, \
         patch.object(ml_request_service.balance_service, "can_withdraw") as mock_can_withdraw, \
         patch.object(ml_request_service.inference_data_service, "add_data", return_value=inference_data_id) as mock_add_inference_data, \
         patch.object(ml_request_service.response_data_service, "add_data") as mock_add_response_data, \
         patch.object(ml_request_service.ml_request_repository, "create_ml_request", return_value=ml_request_id) as mock_create_ml_request, \
        ...
        mock_add_response_data.assert_called_once()
        mock_create_ml_request.assert_called_once_with(model_cost, user_id, model_id, inference_data_id, response_data_id)
